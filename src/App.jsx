...
const GAMES = [
  {
    ...
    inst: {
      ...
      flow: "手番の人が山札を1枚めくって表にします。初めて出た生き物なら、めくった本人がその場で名前を考えて発表します（グループで相談するのではなく、めくった人が決めます）。すでに名前がついた生き物がもう一度出たら、手番かどうかに関係なく、誰でも一番早くその名前を呼べた人が、それまでにめくられて誰も取っていないカードをぜんぶもらいます。カードを取ったら、次の人の手番に移ります。",
      pitfalls: "名前をつけるのは、山札をめくった本人です。自由につけてかまいません、覚えやすくても、ふざけた名前でも大丈夫。早押しで呼べるのは手番の人に限らず、誰でもOKです。正しい名前を言えた人だけがカードを取れて、間違えても基本ペナルティはありません。同時に呼んで判定が難しいときは、その場の空気で決めて大丈夫です（公式な裁定はありません）。",
    },
  },
  {
    ...
    inst: {
      ...
      goal: "自分の前に同じ種類のカードが4枚そろうか、手番で出すカードが手札からなくなった瞬間、その人の負けでゲームは終了します。最後まで負けなかった人たちの勝ちで、細かい順位はつけません。害虫を押しつけ合いながら、自分の前にカードがたまらないように立ち回ります。",
      setup: "カードをよく混ぜて、全員に配りきります。配られたカードは自分の手札として持ちます（枚数が均等に割り切れない場合、数枚多く持つ人がいてもかまいません）。",
      flow: "手番の人は手札から1枚を選び、裏向きのまま相手に差し出して「これは◯◯です」と種類を宣言します（本当でも嘘でもOK）。差し出された人には2つの選べる行動があります。①中身を見ずに、その宣言が本当か嘘かを答える。当たれば差し出した人の前に、外れれば自分の前に、カードを表向きで置きます。②こっそり中身を確認したうえで、まだこのカードを持っていない別の人に、新しい宣言をつけて回す。ただし、一度そのカードを持った人にもう一度回すことはできません。回せる相手がいなくなったら、必ず①を選びます。",
      pitfalls: "宣言する種類は、本当でも嘘でもかまいません。②を選ぶときは中身を見てよいので、見た上でわざと違う名前をつけて回すのも作戦のうちです。同じカードを一度でも持ったことがある人には、二度と回せない点に注意してください（回せる相手がいなければ①を選ぶしかありません）。",
    },
  },
  {
    ...
    inst: {
      ...
      goal: "プラスの点数カードをできるだけ多く取り、マイナスの点数カードをできるだけ人に押しつけて、最終的な点数の合計がいちばん高い人が勝ちです。1〜15の数字カードを1枚ずつ、全部使い切ったらゲーム終了です（＝15ラウンドで終わります）。",
      setup: "各プレイヤーは、1から15の数字カードを1組ずつ持ちます。中央の山には、プラスとマイナスの点数カードをまぜて裏向きに置きます。",
      flow: "場の点数カードを1枚めくって表にします。全員、自分の手札から数字を1枚選び、せーので同時に裏向きから表にします。出した数字はもう手札に戻らず、二度と使えません。プラスの点数カードなら、出した数字がいちばん大きい人がもらいます。マイナスの点数カードなら、いちばん小さい人が引き取ります。同じ数字を出した人どうしがいたら、その数字は相打ちで無効になり、残った中でいちばん強い（高いか低いか）人が対象になります。全員が同じ数字を出して誰も残らなかった場合は、そのカードは誰も取らずに脇に捨てます。",
      pitfalls: "数字カードは一度出したら二度と戻ってきません。全部で15枚なので、ゲームは15ラウンドで終わります。同じ数字どうしは打ち消し合うので、マイナスのカードでは人と数字が被りそうな時にあえて低い数字を投げて無効化を狙う、といった駆け引きも生まれます。",
    },
  },
  {
    ...
    inst: {
      ...
      goal: "4ラウンドを通して、合計得点がいちばん高い人が勝ちです。得点は、出した鳥カードそのものの点数に加えて、鳥の上に置いた卵や、鳥の力で貯め込んだエサ・差し込んだカードにも1点ずつ入ります。使わずに余らせたエサやカードは得点にならないので、盤面に落とし込むことが大切です。",
      ...
      flow: "自分の番には、次の4つから1つだけ行います。1つめ・鳥を出す：手札の鳥に必要なエサを払い、生息地のいちばん左の空きマスに置きます（右のマスに置くときは、追加のコストもかかります）。2つめ・エサを取る：森の列で、バードフィーダーのサイコロからエサを得ます。3つめ・卵を産む：草原の列で、鳥の上に卵を置きます。4つめ・カードを引く：水辺の列で、鳥カードを引きます。エサ・卵・カードは、その列に並んだ鳥が多いほどたくさん得られ、鳥の特殊な力もこのとき発動します。手番はアクションキューブの数だけ行え、ラウンドごとに1つずつ減ります（1ラウンド目は8回、最後は5回）。",
      pitfalls: "行動は1手番に1つだけ。あれもこれもとやりたくなりますが、選んで動きます。鳥は左から順に置くとお得で、右に置くほど追加のコストがかかります。鳥の力は色で発動のタイミングが違う（出したとき・行動したとき・他の人の番・ゲーム終了時）ので、アイコンの色を確かめましょう。エサやカードは、貯め込んだ手元の分だけでは得点にならない点にも注意しましょう。",
    },
    sections: [
      { id: "world", label: "世界観", color: "#8A5BA6",
        body: "あなたは野鳥に魅せられた愛好家。森・草原・水辺という3つの生息地に、さまざまな鳥を呼び寄せていきます。鳥たちはそれぞれ特殊な力を持ち、置くほどに次の手番でできることが増えていく――美しいカードと、コツコツ育っていく手応えが魅力の、拡大再生産ゲームです。" },
      { id: "goal", label: "ゴール", color: "#D8553F",
        body: "4ラウンドを通して、合計得点がいちばん高い人が勝ちです。得点は、出した鳥カードそのものの点数に加えて、鳥の上に置いた卵1個につき1点、鳥の力で貯め込んだエサや差し込んだカード1つにつき1点、選んだボーナスカードとラウンドごとの目標の達成度に応じたボーナス点が入ります。逆に、使わずに余らせたエサやカードは得点になりません。" },
      { id: "setup", label: "準備", color: "#5C9A6B",
        body: "各プレイヤーは個人ボードと、鳥カード5枚・ボーナスカード2枚・エサ5個を受け取ります。ボーナスカードは2枚のうち1枚を選んで残し、もう1枚は伏せて戻します。手元に残したい鳥カード1枚につき、エサを1個捨てます（5枚とも残すなら、エサは5個とも捨てることになります）。場の中央には、エサを出す「バードフィーダー」、表向きの鳥カード3枚、鳥カードの山札、そして4ラウンド分の目標タイルを用意します。" },
      { id: "rounds", label: "ラウンドの流れ", color: "#4F8A73",
        body: "ゲームは全部で4ラウンドです。ラウンドが進むごとに、1人が行える手番の回数（アクションキューブの数）が1つずつ減っていきます（1ラウンド目8回→2ラウンド目7回→3ラウンド目6回→4ラウンド目5回）。各ラウンドの最初には、そのラウンド専用の「ラウンド目標」が公開されます（例：草原の鳥を多く並べる、など）。ラウンドの終わりに目標の達成度を比べ、上位のプレイヤーにボーナス点が入ります。全員のアクションキューブがなくなったらそのラウンドは終わり、次のラウンドに進みます。4ラウンド終わったらゲーム終了です。" },
      { id: "actions", label: "手番のアクション", color: "#3D7EA6",
        body: "自分の番には、次の4つから1つだけ行います。①鳥を出す：カードに書かれたエサ（と、置き場所によっては卵も）を払い、鳥カードに書かれた生息地の列の、いちばん左の空いているマスに置きます。1枚の鳥カードが複数の生息地に対応していることもあり、その場合はどの対応する列に置いてもかまいません。列の右側のマスに置くほど、追加のコストがかかります。②エサを取る：森の列で行います。バードフィーダーのサイコロを振って、出た目を見て、森の列に並んでいる自分の鳥の数だけエサを受け取れます。③卵を産む：草原の列で行います。草原の鳥の数だけ卵を配れますが、1羽の鳥に乗せられる卵の数には、その鳥ごとに上限があります。④カードを引く：水辺の列で行います。場に表向きになっている鳥カードか、山札のいちばん上から、水辺の鳥の数だけカードを引けます。②③④は、その列に並んでいる鳥の数が多いほど得られる量が増え、鳥の特殊な力もこのタイミングで発動します。" },
      { id: "scoring", label: "得点計算", color: "#9C6B3E",
        body: "ゲーム終了時、次をすべて足します。・出した鳥カード本体に書かれている点数・鳥の上にある卵の数（1個につき1点）・鳥の力で貯め込んだエサの数（1個につき1点）・鳥の力で差し込んだカードの数（1枚につき1点）・選んだボーナスカードの達成度に応じた点数・4回分のラウンド目標のボーナス点。エサや手札は、使わずに残っているだけでは得点になりません。鳥の力で実際にボードの上に貯め込んだ分だけが得点になる点に注意してください。" },
      { id: "pitfalls", label: "よくある勘違い", color: "#E2A032",
        body: "行動は1手番に1つだけ。あれもこれもとはできません。鳥は左から順に置くとお得で、右に置くほど追加のコストがかかります。1羽の鳥に置ける卵の数には上限があるので、草原の鳥が多くても、上限に達した鳥にはもう置けません。鳥の力は色で発動のタイミングが違います（出したとき・行動したとき・他の人の番・ゲーム終了時）。エサそのものや、しまってある手札の枚数は得点にならないので、貯め込みすぎず鳥の力を使って盤面に落とし込みましょう。" },
    ],
  },
];
